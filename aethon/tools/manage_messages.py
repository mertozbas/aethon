            ...
            while i < len(messages):
                if messages[i]["role"] == "assistant":
                    ...
                    if i < len(messages) and messages[i]["role"] == "user":
                        has_tool_result = any(
                            "toolResult" in b for b in messages[i].get("content", [])
                        )
                        if has_tool_result:
...
def _remove_tool_blocks(
    messages: List[Dict], tool_ids_to_remove: Set[str]
) -> List[Dict]:
    ...
    for msg in messages:
        ...
        has_thinking = any(
            "thinking" in block or "redacted_thinking" in block
            for block in msg.get("content", [])
        )
    ...
    for idx, msg in enumerate(messages):
        ...
        if idx in pending_by_msg_idx:
            ...
            if next_idx < len(messages):
                ...
                missing_ids = [
                    tid for tid in pending_by_msg_idx[idx] if tid not in next_result_ids
                ]
            else:
            ...
            if missing_ids:
                        ...
                        "content": [
                            {
                                "toolResult": {
                                    ...
                                    "content": [
                                        {
                                            "text": "[conversation modified - synthetic result]"
                                        }
                                    ],
                                }
                            }
                ...
                logger.info(
                    f"Added synthetic toolResult for {len(missing_ids)} orphaned toolUse(s)"
                )
...
def _validate_message_structure(messages: List[Dict]) -> Tuple[bool, str]:
    ...
    if pending_ids:
        return (
            False,
            f"Found {len(pending_ids)} toolUse without toolResult: {pending_ids[:3]}...",
        )
    ...
    for i in range(1, len(messages)):
        ...
        if prev_role == curr_role:
            ...
            return (
                False,
                f"Invalid message sequence at index {i}: {prev_role}→{curr_role}",
            )
...
def manage_messages(
    ...
    if action == "list":
        ...
        if role:
            ...
            for turn_idx, (s, e) in enumerate(turn_list):
                ...
                for msg_idx in range(s, e):
                    ...
                    lines.append(
                        f"  [{msg_idx}] {msg['role']}: {_summarize(msg['content'], summary_len)}"
                    )
        ...
        for i, tc in enumerate(tool_calls):
            ...
            if tc["has_result"]:
                lines.append(
                    f"   Result ({tc['result_status']}): {tc['result_preview'][:50]}..."
                )
            lines.append("")

        lines.append(
            "💡 Use `drop_tools` with `tool_ids` or `tool_name` to remove specific calls"
        )
...
• Pending toolUse (no result): {len(pending_ids)}"""
        ...
        if not path:
            return {
                "status": "error",
                "content": [{"text": "Required: path parameter"}],
            }
        ...
        if not isinstance(imported, list):
            return {
                "status": "error",
                "content": [{"text": "Invalid format: expected list"}],
            }
        ...
        if not is_valid:
            return {
                ...
                "content": [
                    {
                        "text": f"Invalid message structure after fix attempt: {error_msg}"
                    }
                ],
            }
        ...
        if not droppable_messages:
            return {
                "status": "success",
                "content": [
                    {"text": "No droppable messages (only active turn exists)"}
                ],
            }
        ...
        if not turn_list:
            return {
                "status": "success",
                "content": [{"text": "No complete turns found to drop"}],
            }
        ...
        kept = [
            m for i, m in enumerate(droppable_messages) if i not in msg_indices_to_drop
        ]
        ...
        if not tool_ids and not tool_name:
            return {
                ...
                "content": [
                    {
                        "text": "Required: tool_ids (comma-separated) or tool_name parameter"
                    }
                ],
            }
        ...
        if tool_ids:
            for tid in tool_ids.split(","):
                ...
                if tid in active_ids:
                    return {
                        "status": "error",
                        "content": [{"text": f"Cannot drop active tool: {tid}"}],
                    }
                ids_to_drop.add(tid)
        ...
        if not ids_to_drop:
            return {
                ...
                "content": [
                    {
                        "text": f"No droppable tool calls found{' for ' + tool_name if tool_name else ''}"
                    }
                ],
            }
        ...
        if not turn_list:
            return {
                "status": "success",
                "content": [{"text": "No complete turns found to compact"}],
            }
        ...
        if not compact_indices and not turns and start is None and end is None:
            ...
            if len(turn_list) > keep_recent:
                ...
                return {
                    ...
                    "content": [
                        {
                            "text": f"Only {len(turn_list)} turns - nothing to compact (keeping last 3)"
                        }
                    ],
                }
        ...
        tool_blocks_before = sum(
            1
            for m in droppable_messages
            for b in m.get("content", [])
            if "toolUse" in b or "toolResult" in b
        )

        # Strip tool blocks from specified turns
        compacted = _strip_tool_blocks_from_turns(
            droppable_messages, compact_indices, turn_list
        )
        ...
        return {
            ...
            "content": [
                {
                    "text": f"✅ Cleared {turn_count} turns ({droppable_count} messages), preserved active turn"
                }
            ],
        }
