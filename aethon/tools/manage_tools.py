...
def _discover_tools_in_module(
    module_name: str, recursive: bool = True
) -> Dict[str, ToolInfo]:
...
def _format_discovered_tools(
    tools: Dict[str, ToolInfo], module_name: str, verbose: bool = False
) -> str:
...
def manage_tools(
    ...
    if _rt_cfg is not None:
        ...
        if action in _dangerous:
            return {
                ...
                "content": [
                    {
                        "text": f"⚠️ Dynamic tool loading disabled ({ENV_DISABLE_LOAD}=true)"
                    }
                ],
            }
    ...
    if action == "list":
        ...
        text += "\n".join(
            f"  • {t}" + (" (dynamic)" if t in dynamic else "") for t in tool_list
        )
    ...
    # =========================================================================
        ...
        if not code:
            return {
                "status": "error",
                "content": [{"text": "Required: 'code' parameter with Python source"}],
            }
        ...
        try:
            ...
            if not test_result["success"]:
                return {
                    ...
                    "content": [
                        {
                            "text": f"❌ Code failed sandbox test:\n{test_result['output']}"
                        }
                    ],
                }
            ...
            return {
                "status": "error",
                "content": [{"text": "Tool created but failed to load"}],
            }
            ...
            return {
                "status": "error",
                "content": [{"text": f"❌ Failed to create tool: {e}"}],
            }
        ...
        try:
            ...
            if not test_result["success"]:
                return {
                    ...
                    "content": [
                        {
                            "text": f"❌ Fetched code failed sandbox test:\n{test_result['output']}"
                        }
                    ],
                }
            ...
            return {
                "status": "error",
                "content": [{"text": "Tool fetched but failed to load"}],
            }
            ...
            return {
                "status": "error",
                "content": [{"text": f"❌ Failed to fetch tool: {e}"}],
            }
    ...
    # =========================================================================
        ...
        if not tools:
            return {
                ...
                "content": [
                    {
                        "text": "Required: 'tools' parameter with module name (e.g., 'strands_tools')"
                    }
                ],
            }
            ...
            if pkg_name:
                ...
                if install_results.get(pkg_name):
                    try:
                        ...
                        discovered = _discover_tools_in_module(
                            module_name, recursive=True
                        )
                        ...
                        return {
                            ...
                            "content": [
                                {
                                    "text": f"❌ Installed {pkg_name} but still can't import: {e2}"
                                }
                            ],
                        }
                else:
                    return {
                        "status": "error",
                        "content": [{"text": f"❌ Failed to install {pkg_name}"}],
                    }
            else:
                return {"status": "error", "content": [{"text": str(e)}]}
    ...
    # =========================================================================
        ...
        if not code:
            return {
                "status": "error",
                "content": [
                    {"text": "Required: 'code' parameter with Python source to test"}
                ],
            }
        ...
        return {
            ...
            "content": [{"text": f"{status_icon} {msg}"}],
        }
    ...
    # =========================================================================
        ...
        if not tools:
            return {
                "status": "error",
                "content": [
                    {"text": "Required: tools parameter (comma-separated specs)"}
                ],
            }
        ...
        try:
            ...
            if added:
                return {
                    "status": "success",
                    "content": [
                        {"text": f"✅ Added {len(added)} tools: {', '.join(added)}"}
                    ],
                }
            ...
            if not auto_install:
                ...
                return {
                    "status": "error",
                    "content": [{"text": f"❌ Failed to add tools: {e}"}],
                }
            ...
            if not installed:
                return {
                    ...
                    "content": [
                        {
                            "text": f"❌ Failed to install packages: {', '.join(failed_installs)}"
                        }
                    ],
                }
            ...
            try:
                ...
                if failed_installs:
                    msg_parts.append(
                        f"⚠️ Failed to install: {', '.join(failed_installs)}"
                    )
                ...
                return {
                    ...
                    "content": [
                        {
                            "text": f"❌ Installed packages but still failed to load: {e2}"
                        }
                    ],
                }
    ...
    # =========================================================================
        ...
        if not tools:
            return {
                "status": "error",
                "content": [
                    {"text": "Required: tools parameter (comma-separated names)"}
                ],
            }
        ...
        if removed:
            return {
                "status": "success",
                "content": [
                    {"text": f"✅ Removed {len(removed)} tools: {', '.join(removed)}"}
                ],
            }
        return {
            "status": "success",
            "content": [{"text": f"Tools not found: {', '.join(names)}"}],
        }
    ...
    # =========================================================================
        ...
        for tool_name in names:
            ...
            if tool_file:
                try:
                    ...
                    logger.info(
                        f"Reloaded file-based tool: {tool_name} from {tool_file}"
                    )
                except Exception as e:
            ...
            module_path = (
                module if module.endswith(f".{tool_name}") else f"{module}.{tool_name}"
            )
        ...
        return {
            "status": "success" if reloaded else "error",
            "content": [{"text": "\n".join(parts)}],
        }
