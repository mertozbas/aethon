...
### Fixed
- **Cross-channel send timeouts now cancel on Python 3.10.**
  `Future.result(timeout=...)` raises `concurrent.futures.TimeoutError`, which on
  Python 3.10 is a distinct class from `builtins.TimeoutError`; the `except
  TimeoutError` clause missed it there, so a timed-out send fell through to the
  generic error path and was never cancelled (risking a late duplicate delivery).
  Caught the `concurrent.futures` variant explicitly — identical on 3.11+, correct
  on 3.10. (The approval responder already handled this; messaging now matches.)
...
[Unreleased]: https://github.com/mertozbas/aethon/compare/v0.3.1...HEAD
[0.3.1]: https://github.com/mertozbas/aethon/compare/v0.3.0...v0.3.1
